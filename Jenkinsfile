    ...
    stages {
        ...
        stage('deploy dev code') {
            ...
            steps {
                ...
                sh 'ansible-playbook ${ANSIBLE_REPO}/deploy/docker/deploy_docker_compose_dev.yml --extra-vars repo="privateer_stack"'
            }
        }
        // TODO: copy/install/whatever the openvpn and any other sensitive files
        // TODO: source TBD
        stage('install OpenVPN config files') {
            when { branch 'dev_test' }
            steps {
                // deploy configs to DEV
                echo 'install configs'
                sh 'ansible-playbook ${ANSIBLE_REPO}/deploy/docker/TBD.yml'
            }
        }
        ...
        stage('deploy prd code') {
            ...
            steps {
                ...
                sh 'ansible-playbook ${ANSIBLE_REPO}/deploy/docker/deploy_docker_compose_prd.yml --extra-vars repo="privateer_stack"'
            }
        }
        // trigger portainer redeploy
        // separated out so this only gets run if the ansible playbook doesn't fail
